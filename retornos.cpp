		...
		while(auxCol != NULL){
			if(auxNombres->valor == auxCol->nombre){
				if(auxCol->calificador=="PRIMARY KEY"){ //Depues de encontrar la columna con ese nombre, si es pkey
					...
					while(auxPkey!=NULL){ 
						if(auxCol->tipo=="int"){ //Compara los valores de las celdas para saber si se repetirá la pkey
	...
	while(auxCol!=NULL){
		...
		if(noExiste){// Si no hay valor para insertar en una columna de la tupla que se desea insertar
			if(auxCol->calificador != "ANY"){
				...
				nombresCol = insertoPrincipio(nombresCol,auxCol->nombre); // Si es ANY Le pasa la columna que no se especific� y el valor empty
...
TipoRet printDataTable (string  nombreTabla, string ordenadaPor){
	...
	for(int x=0;x<t.tuplas;x++){ // X va desde 0 a t.tuplas-1 para a�adir al arreglo de los indices
		while(auxCelda){
